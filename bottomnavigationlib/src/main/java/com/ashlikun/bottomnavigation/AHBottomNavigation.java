...
public class AHBottomNavigation extends FrameLayout {
    ...
    private void createClassicItems(LinearLayout linearLayout) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (item.isSelect()) {
                icon.setImageDrawable(current ? item.getDrawableSelect(context)
                        : item.getDrawable(context));
            } else {
                icon.setImageDrawable(AHHelper.getTintDrawable(item.getDrawable(context),
    ...
    private void createSmallItems(LinearLayout linearLayout) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (item.isSelect()) {
                icon.setImageDrawable(currentItem == i ? item.getDrawableSelect(context)
                        : item.getDrawable(context));
            } else {
                icon.setImageDrawable(AHHelper.getTintDrawable(item.getDrawable(context),
    ...
    private void updateItems(final int itemIndex, boolean useCallback) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            final AHBottomNavigationItem item = items.get(i);
            ...
            if (i == itemIndex) {
                ...
                if (item.isSelect()) {
                    AHHelper.updateDrawable(context, item.getDrawable(context), item.getDrawableSelect(context)
                            , icon, forceTint);
                } else {
                ...
                if (item.isSelect()) {
                    AHHelper.updateDrawable(context, item.getDrawableSelect(context), item.getDrawable(context)
                            , icon, forceTint);
                } else {
                    AHHelper.updateDrawableColor(context, item.getDrawable(context), icon,
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            if (i == itemIndex) {
                ...
                if (item.isSelect()) {
                    AHHelper.updateDrawable(context, item.getDrawable(context), item.getDrawableSelect(context)
                            , icon, forceTint);
                    icon.setImageDrawable(item.getDrawableSelect(context));
                } else {
                    AHHelper.updateDrawableColor(context, item.getDrawable(context), icon,
                            itemInactiveColor, itemActiveColor, forceTint);
                }
    ...
    private void updateNotifications(boolean updateStyle, int itemPosition) {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            } else if (notificationItem.getText() != null && notificationItem.getText().equals("-1")) {
                notification.setText(" ");
                if (animate) {
    ...
    public void setNotification(int itemPosition) {
        ...
        notifications.set(itemPosition, AHNotification.justText("-1"));
        updateNotifications(false, itemPosition);
    }
